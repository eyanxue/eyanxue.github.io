...
- [Customize](#customize)
  ...
  - [Creating new muscles posts](#creating-new-muscles-posts)
  ...
  - [Removing content](#removing-content)
    - [Removing the muscles page](#removing-the-muscles-page)
...
├── 📂 _posts/: contains the muscles posts
...
## Creating new muscles posts

To create a new muscles post, you can add a new Markdown file in the [\_posts](_posts/) directory, which is the [default location for posts in Jekyll](https://jekyllrb.com/docs/posts/). The [name of the file must follow](https://jekyllrb.com/docs/posts/#creating-posts) the format `YYYY-MM-DD-title.md`. The easiest way to do this is to copy an existing muscles post and modify it. Note that some muscles posts have optional fields in the [frontmatter](https://jekyllrb.com/docs/front-matter/) that are used to enable specific behaviors or functions.

If you want to create muscles posts that are not ready to be published, but you want to track it with git, you can create a [\_drafts](https://jekyllrb.com/docs/posts/#drafts) directory and store them there.
...
You can add extra information to a publication, like a PDF file in the `assets/pdfs/` directory and add the path to the PDF file in the BibTeX entry with the `pdf` field. Some of the supported fields are: `abstract`, `altmetric`, `annotation`, `arxiv`, `bibtex_show`, `muscles`, `code`, `dimensions`, `doi`, `eprint`, `html`, `isbn`, `pdf`, `pmid`, `poster`, `slides`, `supp`, `video`, and `website`.
...
- `muscles`: Adds a "muscles" button redirecting to the specified link
...
Depending on your specified footer behavior, the sign up form either will appear at the bottom of the `About` page and at the bottom of musclesposts if `related_posts` are enabled, or in the footer at the bottom of each page.
...
### Removing the muscles page

To remove the muscles, you have to:

- delete [\_posts](_posts/) directory
- delete muscles page [\_pages/muscles.md](_pages/muscles.md)
- remove reference to muscles page in our [\_pages/dropdown.md](_pages/dropdown.md)
- remove the `latest_posts` part in [\_pages/about.md](_pages/about.md)
- remove the `muscles` section in the [\_config.yml](_config.yml) file and the related parts, like the `jekyll-archives`
...
In order to use this you need to save all of your "Completed" muscles posts which are scheduled to be uploaded on a specific date, in a folder named `_scheduled/` in the root directory.
